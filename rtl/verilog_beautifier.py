...
align_inside_module_comment_line = True
...
class VerilogBeautifier:
    ...
    def beautify_declarations(self, top_ports, lines, start_i, finish_i):
        ...
        """
        if top_ports:
            print(f'Beautifying top ports...({start_i}, {finish_i})')
        else:
            print(f'Beautifying inside module signals declaration...({start_i}, {finish_i})')
        if top_ports:
            indent = ' '*4
            puct = r','
        else:
        ...
        sum_len = len(indent)+dir_max+sig_max+bw_max+bw2_max+name_max+gap_spaces
        if top_ports:
            sum_len = sum_len+1 # +1: ","
        ...
        for i, line in enumerate(key_lines, start_i):
            ...
            if not line.strip():
                ...
                if align_top_ports_comment_line and top_ports: # align comment lines in top ports
                    #assert(re.match(r"\s*//-+\s+(\w+\b\s+)+-+\s*$", line)), f" illegal comment line in top ports: line {i}: {line}"
                    if(re.match(r"\s*//-+\s+(\w+\b\s+)+-+\s*$", line)):
                        line = indent + line.strip()
                        #assert (len(line) <= sum_len), f" Comment too long! sum_len = {sum_len}; len(line) = {len(line)}; line {i}: {line}"
                        if (len(line) <= sum_len):
                            line = line+'-'*(sum_len-len(line))+'\n'
                        else:
                            t = (len(line)-sum_len)*(-1)
                            line = line[:t]+'\n'
                elif align_inside_module_comment_line and (not top_ports): # align comment lines inside module
                    if(re.match(r"\s*//-+\s*[\w\s/]*-+\s*$", line)):
                        line = line.rstrip()
                        if(len(line)<=sum_len):
                            line = line+'-'*(sum_len-len(line))+'\n'
                        else:
                            line_stripped_1st = line[:sum_len]
                            line_stripped_2nd = line[sum_len:]
                            for _ in line_stripped_2nd:
                                assert(_ == '-'), f'Illegal length strip on line {i}: {line}'
                            line = line_stripped_1st+'\n'
                else:
                    line = indent + line.lstrip()
            else:
                line = line.lstrip()
    ...
    def beautify(self, file_path):
        ...
        for i, line in enumerate(lines):
            ...
            if rs and (not inside_declaration):
                if(rs[0] in self.kw_signaltype):
                    start_i = i
                    inside_declaration = True
                    # test:
                    #if start_i==112:
                    #    start_i = start_i-4
                    if align_inside_module_comment_line:
                        start_i = start_i-1
                        while( lines[start_i].strip()=='' or lines[start_i].startswith('//')):
                            start_i = start_i - 1
                        start_i = start_i+1
        ...
        for i, line in enumerate(lines):
            if(i!=len(lines)-1):
                assert(line.endswith('\n')), f'{i}: {line}' # assert each line endswith '\n' except the last line
